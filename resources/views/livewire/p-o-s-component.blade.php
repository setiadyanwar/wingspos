            ...
            <ul class="mb-4 text-black dark:text-white">
                @foreach($cart as $item)
                    <li class="flex items-center justify-between mb-2">
                        ...
                        <img src="{{ asset('storage/' . $item['gambar_produk']) }}" alt="Product Image" class="w-10 h-10 rounded-lg object-cover mr-2" />
                        ...
                        <div class="flex items-center">
                            <button wire:click="updateCart({{ $item->id }}, false)" class="px-2 bg-yellow-500 text-white rounded">-</button>
                            <span class="px-2 text-sm">{{ $cartQuantities[$item->id] ?? 0 }}</span>
